...
let go = () => {
  ...
  exits.forEach(exit => {
    ...
    const directionName = rm.visits > 0 ? `${dir} - ${rm.name}` : dir
...
let talkToOrAboutX = (preposition, x) => {
  ...
  const listTopics = () => {
    ...
    if (topics.length) {
      const availableTopics = topics.filter(topic =>
        topicIsAvailable(character, topic)
      )
      ...
      if (availableTopics.length) {
        availableTopics.forEach(topic =>
          println(
            `${bullet} ${
              topic.option ? topic.option : topic.keyword.toUpperCase()
            }`
          )
        )
        println(`${bullet} ${leaveConversation}`)
      } else {
        endConversation()
      }
      ...
      Object.keys(topics).forEach(topic =>
        println(`${bullet} ${topics[topic].option}`)
      )
    } else {
      endConversation()
    }
  ...
  if (preposition === 'to') {
    ...
    if (!character.topics) {
      println(
        `You have nothing to discuss with ${getName(
          character.name
        )} at this time.`
      )
      return
    ...
    topics =
      typeof character.topics === 'function'
    ...
    if (getCharactersInRoom(room.id).includes(disk.conversant)) {
      ...
      if (response === leaveConversation) {
        ...
        const topic =
          disk.conversation.length &&
          conversationIncludesTopic(disk.conversation, response)
...
let useItem = itemName => {
  const item =
    getItemInInventory(itemName) || getItemInRoom(itemName, disk.roomId)
  ...
  if (item.use) {
    console.warn(
      `Warning: The "use" property for Items has been renamed to "onUse" and support for "use" has been deprecated in text-engine 2.0. Please update your disk, renaming any "use" methods to be called "onUse" instead.`
    )
...
let chars = () => {
  ...
  println(`You see the following:`)
  chars.forEach(char => println(`${bullet} ${getName(char.name)}`))
}

// display help menu
let help = () => {
  const instructions = `Commands with examples:
    LOOK:   'look at key'
    TAKE:   'take book'
    GO:     'go north'
    USE:    'use door'
...
let applyInput = () => {
  ...
  const exec = (cmd, arg) => {
    if (cmd) {
      ...
      println(
        `Sorry, I didn't understand your input. For a list of available commands, type HELP.`
      )
    }
  }
  ...
  if (arguments.length === 1) {
    ...
    disk.conversation &&
    (disk.conversation[command] ||
      conversationIncludesTopic(disk.conversation, command))
  ) {
...
let autocomplete = () => {
  ...
  if (words.length === 1) {
    ...
    if (disk.conversation) {
      ...
      options.push(leaveConversation)
    }
    ...
    const characterNames = (getCharactersInRoom(room.id) || []).map(
  ...
  const matches = (options || [])
    .flat()
    .filter(option => option.match(stubRegex))
  ...
  if (matches.length > 1) {
    ...
    input.value = [
      ...wordsSansStub,
      longestCommonStartingSubstring(matches),
    ].join(' ')
  } else {
...
// string -> item
let getItemInInventory = name =>
  disk.inventory.find(item => objectHasName(item, name))
...
let conversationIncludesTopic = (conversation, keyword) => {
  // leaveConversation is always an option
  if (keyword === leaveConversation) {
    return true
  }
...
let topicIsAvailable = (character, topic) => {
  ...
  const prereqsOk =
    !topic.prereqs ||
    topic.prereqs.every(keyword => character.chatLog.includes(keyword))
  // topic is not removed after read, or it hasn't been read yet
  const readOk =
    !topic.removeOnRead ||
    !character.chatLog.includes(getKeywordFromTopic(topic))
